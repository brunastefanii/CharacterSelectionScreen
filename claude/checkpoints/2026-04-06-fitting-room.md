...
| 7 | Re-attempted with dress centered on body visual center (BODY_CENTER_X = 960.62px, derived from clip percentages) instead of body container left edge — moved dress ~102px to the left | "No, the alignment is not better. You failed." — two attempts at click-to-preview alignment failed. Feature abandoned. | Reverted via `git revert HEAD`. Click-to-preview dress overlay dropped from scope. |
| 8 | Added `@huggingface/transformers` + `Xenova/segformer-b2-clothes` two-step pipeline after `removeBackground()`: remove background → run clothing segmentation → apply canvas mask to zero non-dress pixels; also added hover-to-delete for uploaded dresses (`fr-dress-delete` overlay, `isUploaded` flag, filter from state on click) | "No, it is not working." — feature did not work as expected | Reverted `FittingRoom.jsx`, `FittingRoom.css`, `package.json`, `package-lock.json` to HEAD via `git restore` |
...
| 13 | Added face capture to `placedItems` system to get drag/resize/rotate/× controls — photo rendered as a `fr-placed` div | Image appeared as a square outside the face input area; × button not visible; corner handles not showing | Reverted via `git revert HEAD` |
| 14 | Used `mix-blend-mode: color` overlay div inside `.fr-body` to tint the body silhouette | The blend mode colored the entire bounding box rectangle, not just the silhouette shape — transparent areas around the body turned colored too | Reverted; switched to canvas-based approach with `globalCompositeOperation: 'source-in'` which fills only opaque pixels |
...
- **Canvas `source-in` for silhouette tinting:** Drawing `body.png` to a `<canvas>` then applying `globalCompositeOperation: 'source-in'` fills only the opaque silhouette pixels with the chosen color — transparent areas stay transparent. Body image cached in a ref; redraws are instant on subsequent color changes.
- **Face photo drag/resize/rotate:** Placing the captured face into `placedItems.face` with `clip-path: ellipse(50% 50%)` on the `<img>` only (not `overflow: hidden` on the container) keeps the oval crop while leaving all 4 corner handles, rotation handle, and × button fully visible outside the element bounds.
- **Figma `get_metadata` for exact placement:** When `get_design_context` on the full frame returns stale positions, calling `get_metadata` on the frame node returns the current x/y for every child — used to pin the skin tone picker to `left: 1088px, top: 683px`.
